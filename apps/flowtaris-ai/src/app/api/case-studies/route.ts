 ...
 * Returns all case_studies ordered by published_at desc.
...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('[GET /api/case_studies]', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json(data ?? [], { status: 200 })
  } catch (err: any) {
    console.error('[GET /api/case_studies] Unexpected:', err)
    return NextResponse.json({ error: err.message ?? 'Unknown error' }, { status: 500 })
  }
}

/**
 * POST /api/case_studies
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      console.error('[POST /api/case_studies]', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json(data, { status: 201 })
  } catch (err: any) {
    console.error('[POST /api/case_studies] Unexpected:', err)
    return NextResponse.json({ error: err.message ?? 'Unknown error' }, { status: 500 })
  }
}

/**
 * PATCH /api/case_studies
...
export async function PATCH(request: Request) {
  try {
    ...
    const results = await Promise.all(
      updates.map((u) => 
        client.from('case_studies').update({ sort_order: u.sort_order }).eq('id', u.id)
      )
    )

    const errors = results.filter(r => r.error).map(r => r.error)
    if (errors.length > 0) {
      console.error('[PATCH /api/case_studies] Errors:', errors)
      return NextResponse.json({ error: 'Failed to update some records' }, { status: 500 })
    }

    return NextResponse.json({ success: true }, { status: 200 })
  } catch (err: any) {
    console.error('[PATCH /api/case_studies] Unexpected:', err)
    return NextResponse.json({ error: err.message ?? 'Unknown error' }, { status: 500 })
  }
}
